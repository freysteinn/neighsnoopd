...
GList *netlink_tx_queue = NULL;
bool netlink_tx_in_progress = false;

GList *netlink_cmd_queue = NULL;
...
int netlink_process_rx_queue(void)
{
    ...
    ret = mnl_cb_run(nlh, ret, nlh->nlmsg_seq, mnl_portid,
...
bool netlink_queue_cmd(union netlink_cmd *cmd)
{
    ...
    netlink_cmd_queue = g_list_append(netlink_cmd_queue, cmd_copy);
    if (!netlink_cmd_queue) {
...
union netlink_cmd *netlink_dequeue_cmd(void)
{
    ...
    if (netlink_cmd_queue == NULL)
        return NULL;

    cmd = netlink_cmd_queue->data;
    netlink_cmd_queue = g_list_delete_link(netlink_cmd_queue, netlink_cmd_queue);
    return cmd;
}
...
void netlink_handle_link_linkinfo(struct nlattr *tb[IFLA_MAX + 1], struct netlink_link_cmd *link)
{
...
int netlink_send_neigh(struct neighbor_reply *reply, int ifindex)
{
    ...
    if (reply->vlan_id > 0)
        mnl_attr_put(nlh, NDA_VLAN, sizeof(reply->vlan_id), &reply->vlan_id);
...
int netlink_queue_add(struct nlmsghdr *nlh)
{
    ...
    netlink_tx_queue = g_list_append(netlink_tx_queue, nlh_copy);
...
struct nlmsghdr *netlink_queue_peek(void)
{
    ...
    return netlink_tx_queue->data;
}

struct nlmsghdr *netlink_queue_pop(void)
{
    struct nlmsghdr *nlh = netlink_tx_queue->data;
    netlink_tx_queue = g_list_delete_link(netlink_tx_queue, netlink_tx_queue);
    return nlh;
}
...
int setup_netlink(void)
{
    ...
    // neighbor changes
    if (mnl_socket_bind(nl, RTMGRP_IPV4_IFADDR | RTMGRP_IPV6_IFADDR |
